"""Command line interface for go-ingest."""
import logging
...
def callback(version: bool = typer.Option(False, "--version", is_eager=True),
    ...
    """go-ingest CLI."""
    if version:
        from go_ingest import __version__
        typer.echo(f"go-ingest version: {__version__}")
        raise typer.Exit()


@app.command()
def download(force: bool = typer.Option(False, help="Force download of data, even if it exists")):
    """Download data for go-ingest."""
    typer.echo("Downloading data for go-ingest...")
...
def transform(
    ...
    """Run the Koza transform for go-ingest."""
    typer.echo("Transforming data for go-ingest...")
